...
const ABILITY_REGISTRY = {
  ...
  ground_slam: (unit, ab, { grid, gridSnapshot, sizeR }) => {
    const target = findTargetInLane(unit, gridSnapshot);
    if (target && target.hp > 0) {
...
function processMonsterAbilities(unit, grid, gridSnapshot, sizeR, sizeC) {
  ...
  const context = { grid, gridSnapshot, sizeR, sizeC };
  ...
  for (const ab of stats.abilities) {
    ...
    if (fired) {
      consumedAction = true;
      break; // Only one active ability can consume turn per tick
    }
  }
